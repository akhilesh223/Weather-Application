...
const App = () => {
  ...
  const [unit , setUnit] = useState('C')
   ...
   const API_KEY ='fba23f7b63c7b06d6fcba0603b5dcb1a'
...
const fetchWeatherData = async (url, name = '') => {
  ...
  try {
    ...
    if (!response.ok){
      ...
      //throw new Error
      (
        (await response.json()).message || 'City not Found');
    }
...
// THIS FUNCTIONS PREVENTS FORM SUBMISSION VALIDATES CITY AND FETCHES DATA VIA API
  ...
  return (
    <div className='min-h-screen'>
      ...
      <div className='flex items-center justify-center p-6 min-h-screen'>
        <div className='bg-transparent backdrop-filter backdrop-blur-md rounded-xl shadow-2xl p-8 max-w-md w-full border border-white/30 relative z-10'>
      ...
      {suggestion.length > 0 && (
        <div className='absolute top-12 left-0 right-0 bg-transparent shadow-md rounded z-10'>
          ...
          {suggestion.map((s, index) => (
            <button type='button' key={`${s.lat}-${s.lon}-${index}`}
              onClick={() => fetchWeatherData(
                `https://api.openweathermap.org/data/2.5/weather?lat=${s.lat}&lon=${s.lon}&appid=${API_KEY}&units=metric`,
                `${s.name},${s.country} ${s.state ? `, ${s.state}`:''}`
              )
            } className=' block hover:bg-blue-700 bg-transparent px-4 pt-2 text-sm text-left w-full
               transition-colors'>
                {s.name},{s.country}{s.state && `, ${s.state}`}
...
<div className=' flex flex-wrap justify-around mt-6'>
  {[
  [SunriseIcon, 'Sunrise', weather.sys.sunrise], [SunsetIcon, 'Sunset', weather.sys.sunset]
  ] .map(([Icon, label, time]) => (
    <div key={label} className=' flex flex-col items-center m-2'>
      <Icon />
      <p className=' mt-1 font-semibold'>{label}</p>
